...
transform transition-transform duration-300 ease-in-out ${
    ...
    >
      <div className="flex items-center justify-center h-16 border-b border-border bg-white px-4 md:px-2">
      ...
      <nav className="flex-1 mt-6 px-4 space-y-2 overflow-y-auto no-scrollbar md:pr-2">
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${
              isActive(item.path)
                ...
                : "text-gray-600 hover:bg-gray-50 hover:text-primary hover:pl-5 md:hover:pl-4"
            }`}
          >
            {item.icon}
            <span>{item.name}</span>
